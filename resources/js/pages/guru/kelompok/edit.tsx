...
export default function EditKelompok() {
    const { proyek, ketuaCandidates, kelompok } = usePage().props as { proyek?: Proyek; ketuaCandidates?: User[]; kelompok?: Kelompok };
    ...
    const ketuaOptions = [
        ...(kelompok?.ketua
            ? [
                  {
                      value: kelompok.ketua_id,
                      label: kelompok.ketua,
                  },
              ]
            : []),
        ...(ketuaCandidates
            ?.filter((candidate) => candidate.id.toString() !== kelompok?.ketua_id)
            .map((ketua) => ({
                value: ketua.id,
                label: ketua.name,
            })) || []),
    ...
    const { data, setData, patch, processing, errors } = useForm<Required<KelompokForm>>({
        nama: kelompok?.nama ?? '',
        jumlah_anggota: kelompok?.jumlah_anggota ?? 0,
        ketua_id: kelompok?.ketua_id ?? '',
